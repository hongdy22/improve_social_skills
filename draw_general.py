...
def plot_sections(section1, section2):
    """绘制折线图"""
    plt.figure(figsize=(10, 5))
    x = np.arange(1, section1.shape[1] + 1)
    
    labels = ["Objective Evaluation", "Self Evaluation", "Peer Evaluation"]
    for i in range(section1.shape[0]):
        plt.plot(x, section1[i], marker='o', linestyle='-', label=f'Difficult section - {labels[i]}')
    for i in range(section2.shape[0]):
        plt.plot(x, section2[i], marker='s', linestyle='--', label=f'Easy section - {labels[i]}')
...
def main():
    ...
    # 绘制折线图
    plot_sections(section1, section2)
